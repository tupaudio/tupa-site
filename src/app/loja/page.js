...
export default function Loja() { // Renomeado para Loja para consistência
  const [categoria, setCategoria] = useState('Todos');
  const [tecnologia, setTecnologia] = useState('Todos');

  // Extração automática para os filtros
  const categorias = ['Todos', ...new Set(produtos.map(p => p.categoria))];
  const tecnologias = ['Todos', ...new Set(produtos.map(p => p.tecnologia))];

  // A variável 'filtrados' deve estar DENTRO da função do componente
    ...
    <main className="min-h-screen bg-tupaBlack p-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
        {filtrados.map((produto) => (
          <AmplifierCard 
            key={produto.id} 
            produto={produto} // Passando o objeto inteiro aqui
          />
        ))}
      </div>
    </main>
